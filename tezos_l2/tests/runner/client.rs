use context::{ExecutorContext, GenericContext, InterpreterContext, migrations::run_migrations};
use std::collections::HashMap;
use tezos_core::types::{
    encoded::{ImplicitAddress, PublicKey, Ed25519Seed, Ed25519PublicKey, SecretKey, Ed25519SecretKey, OperationHash, Ed25519PublicKeyHash, Encoded},
    number::Nat,
};
use tezos_core::internal::crypto::blake2b;
use tezos_michelson::micheline::Micheline;
use tezos_operation::operations::{SignedOperation, OperationContent, Reveal, UnsignedOperation, Origination, Transaction, Script, Parameters, Entrypoint};

use tezos_l2::{
    producer::batch::apply_batch
};
...
impl Wallet {
    fn from_keypair(keypair: ed25519_dalek::Keypair) -> Self {
        ...
        let key_hash = Ed25519PublicKeyHash::from_bytes(digest.as_slice())
...
impl<Context: GenericContext + ExecutorContext + InterpreterContext> Client<Context> {
    ...
    pub fn migrate(&mut self) {
        let head = self.context.get_head()
            .expect("Failed to get head");

        run_migrations(&mut self.context, &head)
            .expect("Failed to run context migrations");
    }
    ...
    pub fn import_wallet(&mut self, alias: &'static str, sk: &'static str) {
        self.wallets.insert(
            alias.into(), 
            Wallet::from(Ed25519Seed::new(sk.into())
    ...
    pub fn get_balance(&mut self) -> u32 {
        ...
        self.context.get_balance(wallet.address.value())
            .unwrap()
    ...
    pub fn get_contract_storage(&mut self, address: &str) -> String {
        let storage = self.context.get_contract_storage(address).unwrap()
    ...
    pub fn topup(&mut self, balance: u32) {
        ...
        self.context.set_balance(wallet.address.value(), &balance.into()).unwrap();
    }
    ...
    pub fn reveal(&mut self) -> &mut Self {
        ...
        self.group.push(OperationContent::Reveal(
            Reveal {
    ...
    pub fn originate(
        ...
        self.group.push(OperationContent::Origination(
            Origination {
                source: wallet.address.clone(),
                counter: wallet.get_counter(),
    ...
    pub fn transfer(&mut self, destination: &str, amount: u32) -> &mut Self {
        ...
        self.group.push(OperationContent::Transaction(
            Transaction {
                source: wallet.address.clone(),
                counter: wallet.get_counter(),
                amount: amount.into(),
    ...
    pub fn inject(&mut self) -> OperationHash {
        ...
        // println!("{:#?}", operation);
        let signature = operation.sign(&wallet.secret_key)
    ...
    pub fn bake(&mut self) {
        ...
        apply_batch(&mut self.context, head, payload, true)
